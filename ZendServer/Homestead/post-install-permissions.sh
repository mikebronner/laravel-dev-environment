...
sed -i -e 's/^user =.*/user = vagrant/' /usr/local/zend/etc/php-fpm.conf
sed -i -e 's/^listen.owner =.*/listen.owner = vagrant/' /usr/local/zend/etc/php-fpm.conf
...
#find /usr/local/zend -group nginx -exec chgrp vagrant {} \;
